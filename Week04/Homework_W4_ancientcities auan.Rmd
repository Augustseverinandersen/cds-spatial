...
# I can not get the Knit file to show output?
...
st_crs(cities_sf3035) # IT is using meters and not degrees therefor it is projected
```
...
*1. What are the measurement units of the `cities_sf3035` object?*
 ...
 - Meters. by using st_crs you can check the "lengthunit"
...
Arguments retained: tm_shape, tm_polygons, tm_dots

Arguments dropped: everything else 
...
i_sm <- inscriptions %>% 
  ...
  mutate(latitude = as.numeric(gsub("\\)", "",latitude)), # using gsub to replace  the literal )
         longitude = as.numeric(gsub("c\\(", "",longitude))) # using gsub to replace c and the literal ( with nothing
...
leaflet() %>% addTiles() %>% addMarkers(lng=i_sm$longitude,lat=i_sm$latitude) # the 12000 looked good, but my computer nearly
# died.  So I have changed the plotting to only the first 100 rows
```
...
- From what I could see before my computer nearly died, southern Europe. 
...
i <- inscriptions %>% 
  #slice(1:100) %>% I first tested on 100 rows
  separate( col = coordinates, into = c("longitude","latitude"), sep = ",") %>% 
  mutate(latitude = as.numeric(gsub("\\)", "",latitude)), # using gsub to replace  the literal )
         longitude = as.numeric(gsub("c\\(", "",longitude))) %>% 
  filter(!is.na(longitude)) %>%  # checing for NAs, but using ! to reverse TRUE and FALSE
...
st_crs(insc_sf4326)# checing to see if the crs is now 4326
```
...
crs(insc_sf3035)==crs(cities_5km) # TRUE
...
cities_it <- st_intersection(insc_ch, st_intersection(cities_5km))
...
```{r ration calculation, eval=FALSE}
number_urban <- nrow(insc_urban) # finding number of rows 
number_rural <- nrow(insc_rural) # finding number of rows
ratio_rual <- number_rural / number_urban # calculating ration
ratio_urban <- number_urban / number_rural
ratio_urban_txt <- paste("For every rural inscription, there is", ratio_urban, "urban inscriptions")
ratio_rural_txt <- paste("For every urban inscription, there is", ratio, "rural inscriptions")
ratio_rural_txt # 0.15 rural inscription pr urban inscription
ratio_urban_txt # 6.5 urban inscription pr rural inscription 
```
