...
unique(monuments$datering) # Oldtid, Stenalder, Bronzealder, Jernalder, Middelalder, Historisk Tid, Vikingetid, Udateret, Nyrere tid, Efterreformatorisk tid, NA
...
municipalities_25832 <- municipalities %>% 
  ...
  st_transform(municipalities_25832, crs = 25832) # projecting the new crs.

crs(municipalities_25832) # checking to see if the crs is now 25832
...
class(viking) # has class sf, tbl_df, tbl, data.frame 
...
viking_neigh %>% 
  group_by(NAME_2) %>% 
  tally() %>% 
  arrange(desc(n))  #Bornholm, Esbjerg, THisted, Lolland, Kerteminde 
...
plot(municipalities_25832%>%
       filter(NAME_2 %in% least$NAME_2) %>% # looking for matching NAME_2
...
nb_buf <- nb %>% 
  st_as_sf(coords = c("longitude", "latitude"), crs = 4326) %>% # define crs 
  st_transform(crs = 25832) %>%  # project crs 
  st_buffer(10000)


# Check consistent projection among the objects
crs(nb_buf)==crs(viking)
crs(viking)==crs(parks)
...
2. Why do we use `parks[cont,]` to view the fully contained parks in the buffer plot?
  cont is a new colunm created in parks. by not adding anythin after , we get everything 
...
nearest10index <- which( d %in% sorted[1:10] ) # which distances are smaller than the 10th smallest distance
...
# Joinning
kirke_mun <- st_join(kirke, municipalities_25832)


# summerzing and group by
kirke_sum <- kirke_mun %>%
  group_by(NAME_2) %>%
  summarize(count = n())

#Chloropleth map
kirke_map <- tm_shape(kirke_sum) +
  tm_dots("count") +
  tm_borders("NAME_2", col = "black")
