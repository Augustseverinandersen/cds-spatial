...
tm_shape(nz_elev)  +
  ...
  tm_compass(position = c("RIGHT", "top"),
             type = "rose", 
             size = 2) +
  tm_credits(text = "A. Andersen, 2023") +
...
head(zion)
# Zion contains 12 coloums that store information abut the national park, metadata and geoms. 
class(zion)
# It stores the following datatypes: sf, tibble dataframe, tibble, and dataframe. 
crs(zion)
# the coordinate system used is ESPG 8610 and which uses the 1927 DATUM of North America. UTM Zone 12.
zion
# 1 or 11
st_geometry(zion)
# It is a POLYGON
...
# unsure
class(srtm)
# is stores the following data in layers: Raster 
crs(srtm)
# It is ESPG 9122. Datum is World Geodetic System 1983, 
srtm
# i dont know how many attributes  1 or 11
dim(srtm)
# the dimension are 457, 465, and 1 I get three
res(srtm)
# the resolutions is x = 0.00083, and y = 0.00083
...
# getting the crs
the_crs <- crs(srtm, asText = TRUE) # asText gives a Well Known Text Format
# projecting the crs 
zion_9122 <- st_transform(zion, crs = the_crs)
# Checking if the crs is the same now
all.equal(crs(zion_9122), crs(srtm)) # It does not give true or false

# Plotting the first crs visualisation
plot(st_geometry(zion), col = "grey", border = "white")
plot(srtm, add = TRUE, pch = 16, col = "forestgreen")

new_crs <- crs(zion) # taking the crs from zion (I think)

# Projecting the crs
srtm_8610 <- projectRaster(srtm, crs = new_crs) 
