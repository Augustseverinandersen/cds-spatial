...
my_token <- "pk.eyJ1IjoiYXVndXN0YW5kZXJzZW4iLCJhIjoiY2xld3lwM29vMGpmYzN3a2NnZ2JpZjIyeSJ9.7nWPzFc35SPZTBSvhN_iRQ"
...
home <- mb_geocode("Jægergårdgade 126, Aarhus, 8000, Denmark")
home
...
```{r}
## An isochrone map around a location of your choice 
home_iso_drive <- mb_isochrone(home,
                                 profile = "driving",
                                 time = c(10, 20, 30))
colors <- viridisLite::viridis(3)
...
leaflet() %>%
  addMapboxTiles(style_id = "outdoors-v11",
                 username = "mapbox") %>%
  addPolygons(data =home_iso_drive, 
              color = rev(colors),
              fillColor = rev(colors),
              fillOpacity = 0.5, 
              opacity = 0.5, 
  ...
  addLegend(labels = c(10, 20, 30),
            colors = colors,
            title = "Drive-time<br/>around home")

## A route between two locations of your choice
myroute <- mb_directions(origin = home,
                       destination = "Rådhuspladsen, Copenhagen, Denmark",
                       profile = "cycling")
...
mapbox_map %>%
  addPolylines(data = myroute, 
               popup = paste0(
                 "Distance (km): ",
                 round(myroute$distance, 1), 
                 "<br/>Time (minutes): ",
                 round(myroute$duration, 1)
               ))
```
