...
Forests: 32 features, Geometry: Multipolygon
Parks: 47 features, Geometry Multipolygon
Playgrounds: 51 features, Geometry: Point
Shelters: 19 features, Geometry Point 

2. What is the CRS value in these objects?
Forests: ETRS89 / UTM zone 32N
Parks: ETRS89 / UTM zone 32N
Playgrounds: NA
Shelters: ETRS89 / UTM zone 32N
...
bigParks <- filter(parks, areal_m2 > 50000)
nrow(bigParks) # counting nbr of rows
selectedParks <- bigParks %>% select(navn, areal_m2) # selecting only navn and areal columns
...
big_forests <- forests %>% filter(unclass(areas) > 30000)
...
plot(st_geometry(big_forests["geometry"]))
...
plot(forests_geo, col = "green");plot(parks$geometry, col = "red", add = TRUE)
...
3 columns and 1 row with multiple under rows
2. What is the resolution of `aarhus` orthophoto? How many layers does it contain and what do they represent?
5 it contains the 3 color layers of Red Green and Blue. The other two I don't know.
